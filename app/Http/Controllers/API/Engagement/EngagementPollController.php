...
use App\Models\Engagement\EngagementPollVote;
...
class EngagementPollController extends Controller
{
    public function index()
    {
        $polls = EngagementPostEvent::where('type', 'poll')
            ->withCount([
    ...
    {
        $total_polls     = EngagementPostEvent::where('type', 'poll')->count();
        $total_votes     = EngagementPollVote::count();
        $total_employees = User::where('role', User::ROLE_EMPLOYEE)->count();
        $unique_voters   = EngagementPollVote::distinct('user_id')->count('user_id');

        $most_selected = EngagementPollOption::leftJoin(
                'engagement_poll_votes',
                'engagement_poll_votes.engagement_poll_option_id',
                '=',
                'engagement_poll_options.id'
            )
            ->select(
                'engagement_poll_options.label',
                DB::raw('COUNT(engagement_poll_votes.id) as vote_count')
            )
            ->groupBy('engagement_poll_options.id', 'engagement_poll_options.label')
            ->orderBy('vote_count', 'desc')
            ->first();

        $highest_engagement = EngagementPostEvent::where('type', 'poll')
            ->withCount('pollVotes')
    ...
    {
        $poll = EngagementPostEvent::where('id', $id)->where('type', 'poll')->first();
        ...
        $poll->load(['pollOptions' => fn ($q) => $q->orderBy('sort_order'), 'pollVotes']);
        ...
        $results = $poll->pollOptions->map(function ($option) use ($poll, $total_votes) {
            $vote_count = $poll->pollVotes
                ->where('engagement_poll_option_id', $option->id)
                ->count();
    ...
    public function vote_records($id)
    {
        ...
        $records = EngagementPollVote::with(['option:id,label', 'user:id,name'])
            ->where('engagement_post_event_id', $poll->id)
            ->orderBy('created_at', 'desc')
            ->get()
    ...
    {
        $poll     = EngagementPostEvent::where('id', $id)->where('type', 'poll')->firstOrFail();
        ...
        return response()->streamDownload(function () use ($poll) {
            ...
            EngagementPollVote::with(['option:id,label', 'user:id,name'])
                ->where('engagement_post_event_id', $poll->id)
                ->orderBy('created_at', 'desc')
    ...
    public function vote(Request $request, $id)
    {
        ...
        $request->validate([
            'option_id' => 'required|integer|exists:engagement_poll_options,id',
        ]);

        $option = EngagementPollOption::where('id', $request->option_id)
            ->where('engagement_post_event_id', $poll->id)
            ->first();
        ...
        EngagementPollVote::updateOrCreate(
            [
                'engagement_post_event_id' => $poll->id,
                'user_id'                  => Auth::id(),
            ],
            [
                'engagement_poll_option_id' => $option->id,
            ]
        );
        ...
        return response()->json([
            'data' => [
                ...
                'options'           => $poll->pollOptions->map(function ($opt) use ($poll, $total_votes) {
                    $vote_count = $poll->pollVotes
                        ->where('engagement_poll_option_id', $opt->id)
                        ->count();
                    return [
                        'id'         => $opt->id,
                        'label'      => $opt->label,
