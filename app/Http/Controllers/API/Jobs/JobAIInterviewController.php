...
use App\Models\Jobs\JobAIInterviewQna;
use App\Services\JobAIInterviewService;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Storage;

class JobAIInterviewController extends Controller
{
    ...
    {
        $data = JobAIInterview::where('id', $id)->with(['answers'])->first();
        return response()->json($data);
    }
    public function start(Request $request, JobAIInterviewService $ai)
    {
        ...
        $ji = JobAIInterview::where('id', $request->job_interview_id)->first();
        if ($ji) {
            $ji->update(['status' => 'in_progress']);
        }
        JobAIInterviewQna::where('job_a_i_interview_id', $request->job_interview_id)->delete();
        foreach ($questions as $qText) {
            $qna = JobAIInterviewQna::create([
                'job_a_i_interview_id' => $request->job_interview_id,
                'question' => $qText
            ]);
    ...
    public function submitAnswer(Request $request, $id, JobAIInterviewService $ai)
    {
        $interview = JobAIInterview::findOrFail($id);
        $answers = $request->input('answers');
        $results = [];

        foreach ($answers as $index => $data) {
            $qna = JobAIInterviewQna::where('job_a_i_interview_id', $id)->find($data['qna_id']);
            if (!$qna) continue;
