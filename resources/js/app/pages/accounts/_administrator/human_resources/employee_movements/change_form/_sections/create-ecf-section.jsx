...
import {
  get_employee_change_form_thunk,
  search_employee_thunk,
...
const POSITION_LEVEL_OPTIONS = ["Rank and File", "Supervisor", "Manager", "Executive"].map((level) => ({
  label: level,
  value: level,
}));

const STATUS_OPTIONS = ["Probationary", "Regular"].map((status) => ({
  label: status,
  value: status,
}));
...
const CreateECFSection = () => {
  ...
  const { employee } = useSelector((store) => store.human_resources);
  ...
  const watchedReportingIdTo = watch("info_reporting_id_to");

  // Debounced Employee Search
  const handleSearchEmployee = useCallback((value) => {
  ...
  // Hydrate Form on Employee Selection
  useEffect(() => {
    if (!selected) return;

    const leaderInfo = selected?.er_leader?.employee;
    const personalInfo = selected?.personal_information;
    const reportingName = `${leaderInfo?.personal_information?.first_name || ""} ${leaderInfo?.personal_information?.last_name || ""}`.trim();
    const employeeName = `${personalInfo?.first_name || ""} ${personalInfo?.last_name || ""}`.trim();
    ...
    reset({
      ...
      hire_date: selected?.started_at,
      position_level: selected?.position_level ?? "N/A",
      info_position_level_from: selected?.position_level ?? "N/A",
      info_position_level_to: selected?.position_level ?? "N/A",
      info_department_from: selected?.department?.name,
      info_department_id_from: selected?.department?.id,
      info_department_id_to: selected?.department?.id,
      info_account_from: selected?.account?.name,
      info_account_id_from: selected?.account?.id,
      info_account_id_to: selected?.account_id,
      info_status_from: selected?.status,
      info_status_to: selected?.status,
      info_position_from: selected?.position,
      info_position_to: selected?.position,
      info_reporting_from: reportingName,
      info_reporting_id_from: leaderInfo?.e_r_leader_id,
      info_reporting_id_to: leaderInfo?.e_r_leader_id,
      info_basic_pay_from: selected?.basic_pay,
      info_basic_pay_to: selected?.basic_pay,
  ...
  // Update target reporting manager name when manager selection changes
  useEffect(() => {
    if (!watchedReportingIdTo) return;

    const newReport = data?.leaders?.find((res) => res.id === watchedReportingIdTo);
    if (newReport) {
      const leaderName = `${newReport?.user?.personal_information?.first_name || ""} ${newReport?.user?.personal_information?.last_name || ""}`.trim();
      setValue("info_reporting_to", leaderName);
    }
  }, [watchedReportingIdTo, data?.leaders, setValue]);

  const onSubmit = async (formData) => {
    try {
      await create_employee_change_form_service(formData);
      ...
      console.error("Error creating Employee Change Form:", error);
    }
  };
    ...
    <>
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)} className="mx-auto p-8 text-black font-sans">
          ...
          <div className="mb-6">
            <h2 className="font-bold text-sm mb-1">General Employee Information:</h2>
          ...
          <div className="mb-6 flex items-start flex-col gap-3 w-full text-sm">
            ...
            <div className="font-bold w-full">
              <TextArea
                ...
                type="text"
                {...register("reason_for_change", { required: "Required!" })}
                className="bg-transparent w-full outline-none text-black"
                error={errors?.reason_for_change?.message ?? ""}
                onChange={(val) => setValue("reason_for_change", val.target.value)}
              />
            </div>
            <div className="flex gap-3 items-start justify-center w-full">
              {selected?.status === "Probationary" && (
                <Checkbox
                  label="Regular"
                  {...register("regular")}
          ...
          <div className="my-6">
            <h2 className="font-bold uppercase text-sm mb-1">New Information Details:</h2>
                ...
                <tr>
                  ...
                  <td className="border border-black p-1">
                    <Select
                      ...
                      onChange={(val) => setValue("info_position_level_to", val)}
                ...
                <tr>
                  ...
                  <td className="border border-black p-1">
                    <Select
                      ...
                      onChange={(val) => setValue("info_department_id_to", val)}
                ...
                <tr>
                  ...
                  <td className="border border-black p-1">
                    <Select
                      ...
                      onChange={(val) => setValue("info_account_id_to", val)}
                ...
                <tr>
                  ...
                  <td className="border border-black p-1">
                    <Select
                      ...
                      onChange={(val) => setValue("info_status_from", val)}
                  ...
                  <td className="border border-black p-1">
                    <Select
                      ...
                      onChange={(val) => setValue("info_status_to", val)}
                  ...
                  <td className="border border-black p-1">
                    <Select
                      ...
                      options={data?.leaders?.map((res) => ({
                        label: `${res?.user?.personal_information?.first_name || ""} ${res?.user?.personal_information?.last_name || ""}`.trim(),
                        value: res?.id,
                      }))}
                      value={watchedValues?.info_reporting_id_to}
                      onChange={(val) => setValue("info_reporting_id_to", val)}
            ...
            <div className="my-6">
              <div className="flex justify-between items-center mb-2">
                <h2 className="font-bold uppercase text-sm">Allowances:</h2>
                <Button
                  type="button"
          ...
          <div className="text-center mt-12 mb-8">
            ...
            <p className="text-[10px] text-gray-400 italic text-center leading-tight">
              Disclaimer:
              <br />
              This document and its contents are the property of EmpireOne BPO Solutions, Inc. and
              are intended for internal use only. Unauthorized reproduction, disclosure, or
              distribution of this material, in whole or in part, without prior written permission
