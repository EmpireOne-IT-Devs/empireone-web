...
// You might need to import an update service here instead of create, depending on your backend!
...
const EvaluationFormSection = ({ isEdit = false }) => {
    ...
    useEffect(() => {
        ...
        setValue(
            "supervisor_id",
            `${evaluation?.supervisor?.personal_information?.user_id}`,
        );
        setValue("user_id", `${evaluation?.user?.id}`);
        setValue(
            "supervisor_name",
        ...
        setValue(
            "date_of_assessment",
            moment(evaluation?.date_of_assessment).format("YYYY-MM-DD"),
        );
        setValue("recommendation", evaluation?.recommendation);
        setValue("remarks", evaluation?.remarks);

        if (evaluation?.section1s) {
            setValue(
                "objectives",
                evaluation?.section1s?.map((res) => ({
                    // Also sending ID to backend for accurate row updates
                    id: res.id,
                    title: res.objective,
    ...
    const onSubmit = async (data) => {
        const finalPayload = {
            evaluation_id: evaluation?.id, // Passing the ID so backend knows which record to update
            ...data,
            calculated_scores: {
        ...
        try {
            // NOTE: If your backend has a separate Update route, swap this service call out!
            ...
            dispatch(
                setAlert({
                    ...
                    message: "The Evaluation changes have been saved.",
                    open: true,
                }),
            );
        } catch (error) {}
    ...
    // CHANGED: Added disabled prop to pass down from isEdit
    const RatingRadioGroup = ({ name, hasError, disabled }) => (
        <div
        ...
        >
            {[1, 2, 3, 4, 5].map((val) => (
                <label
                    key={val}
                    className={`flex flex-col items-center group ${disabled ? "cursor-not-allowed opacity-70" : "cursor-pointer"}`}
                >
    ...
    const inputStyles =
        "w-full p-2 border-b-2 border-transparent bg-yellow-50 hover:bg-yellow-100 focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all duration-200 outline-none rounded-sm disabled:bg-gray-50 disabled:text-gray-600 disabled:cursor-not-allowed disabled:border-transparent";
        ...
        <div className="max-w-4xl mx-auto p-8 bg-white shadow-2xl border border-gray-200 my-10 rounded-lg">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-[1px] bg-gray-300 border border-gray-300 rounded-sm overflow-hidden shadow-sm">
                    ...
                    <input
                        disabled // Always disabled to prevent changing the linked user
                    ...
                    <input
                        disabled={!isEdit} // Editable if in edit mode
                        type="date"
                        {...register("date_of_assessment", { required: true })}
                ...
                >
                    ...
                    <textarea
                        ...
                        className={`w-full p-3 border rounded outline-none transition-all min-h-[120px] ${getErrorStyle(errors.remarks) || "border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent"} disabled:bg-gray-100 disabled:cursor-not-allowed`}
                    />
                </div>
                ...
                <div className="mt-12 grid grid-cols-2 gap-12 px-6">
                    ...
                    <div className="flex flex-col items-center justify-end h-full w-full max-w-sm mx-auto">
                        <div className="relative flex flex-col items-center justify-end w-full">
                            {evaluation?.supervisor?.account_employee
                                ?.signature && (
                                <img
                                    src={
                                        evaluation?.supervisor?.account_employee
                ...
                {/* Conditionally render Submit Button only if in Edit mode */}
                {isEdit && (
