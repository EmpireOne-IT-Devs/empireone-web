    ...
    // API Services
    import { toggle_engagement_reaction_service } from "@/app/services/engagement-service";
    ...
    export default function NewsSection() {
        ...
        async function handleReact(e, item) {
            ...
            try {
                const res = await toggle_engagement_reaction_service(item.id);
                const { reaction_count, user_has_reacted } = res.data.data;
                dispatch(syncInteraction({ postId: item.id, reaction_count, user_has_reacted }));
            } catch {
                // Rollback on error
                dispatch(
                    syncInteraction({
        ...
        const newsItems = postArray
            .filter((p) => p.category === "News")
            .map((p) => {
                const rawMessage = p.message || p.content || "";
                const rawTitle = p.headline || p.title || "Untitled News";
                ...
                return {
                    ...
                    title: rawTitle,
                    description: stripHtml(rawMessage),
