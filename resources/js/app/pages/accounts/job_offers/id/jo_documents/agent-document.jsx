...
const OfferLetterPDF = (data) => {
    // Safely parse the medical benefits array before rendering
    const medicalBenefits = Array.isArray(data?.medical_benefits)
        ? data.medical_benefits
        : typeof data?.medical_benefits === "string"
            ? JSON.parse(data.medical_benefits || "[]")
            : [];

    const hasMedicalBenefits = medicalBenefits.length > 0;
            ...
            <Page size="A4" style={styles.page}>
                ...
                <View style={styles.row}>
                    ...
                    <Text style={styles.value}>
                        Upon your {data?.effective_period?.toLowerCase()} {data?.annual_leave} leave credits to be utilized as VL/SL/EL{"\n"}
                ...
                {hasMedicalBenefits && (
                    <>
                        ...
                        {medicalBenefits.includes("Hospitalization") && (
                            <ListItem>
                                <View style={styles.row}>
                        ...
                        {medicalBenefits.includes("Dental") && (
                            <ListItem>
                                <View style={styles.row}>
                        ...
                        {medicalBenefits.includes("Dependent") && (
                            <ListItem>
                                <View style={styles.row}>
                        ...
                        {medicalBenefits.includes("Group Life Insurance") && (
                            <ListItem>
                                <View style={styles.row}>
                                    <Text style={[styles.label]}>
                                        Group Life Insurance up to Php200,000 (for Principal only)
                                    </Text>
                                </View>
                            </ListItem>
                        )}
...
const AgentOfferLetterPreview = ({ name, type, applicant_signature }) => {
    ...
    const rawData = {
        ...
        effective_period: job_offer?.effective_period, // Added to pass effective period down
