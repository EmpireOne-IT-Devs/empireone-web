...
export const useInterviewLogic = () => {
    const { job_interview } = useSelector((store) => store.app);
    ...
    // console.log(
    //     "job_interview",
    //     job_interview?.answers.map((res) => res.question),
    // );
    ...
    useEffect(() => {
        if (job_interview?.status == "completed") {
            setIsFinished(true);
        }
        if (job_interview?.status == "in_progress") {
            setJobTitle(job_interview?.job_title);
            startInterview();
        }
        if (!audioRef.current) {
    ...
    const startInterview = async () => {
        ...
        try {
            const res = await axios.post("/api/interviews/start", {
                job_title: job_interview?.job_title,
                job_interview_id: job_interview?.id,
            });
