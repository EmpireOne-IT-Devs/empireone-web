...
import { TbBriefcase, TbSearch, TbSend, TbPaperclip, TbDotsVertical, TbPhone, TbVideo, TbChevronLeft, TbCheck, TbChecks } from "react-icons/tb";
...
const CONVERSATIONS = [
    {
        ...
        initials: "SD",
        lastMessage: "Thank you for applying! We have reviewed your application...",
        time: "2 days ago",
        unread: 2,
        online: true,
        messages: [
            { id: 1, from: "them", text: "Hello! Thank you for applying to the Software Developer position at our company.", time: "10:00 AM", read: true },
            { id: 2, from: "them", text: "We have reviewed your application and we're impressed with your background.", time: "10:01 AM", read: true },
            { id: 3, from: "me", text: "Thank you so much! I'm really excited about this opportunity.", time: "10:15 AM", read: true },
            { id: 4, from: "me", text: "I've been following your company for a while and I believe my skills in React and Node.js would be a great fit.", time: "10:16 AM", read: true },
            { id: 5, from: "them", text: "Great! We'd love to schedule an initial interview. Are you available this week?", time: "10:30 AM", read: true },
            { id: 6, from: "me", text: "Absolutely! I'm available Thursday or Friday afternoon.", time: "10:45 AM", read: true },
            { id: 7, from: "them", text: "Perfect. Let's set it for Friday at 2 PM. We'll send a calendar invite shortly.", time: "11:00 AM", read: true },
            { id: 8, from: "them", text: "Thank you for applying! We have reviewed your application and would like to move forward.", time: "2 days ago", read: false },
        ],
    },
    {
        id: 2,
        ...
        online: false,
        messages: [
            { id: 1, from: "them", text: "Hi! We received your application for the Product Designer role.", time: "9:00 AM", read: true },
            { id: 2, from: "them", text: "Could you share your portfolio link?", time: "9:05 AM", read: true },
            { id: 3, from: "me", text: "Of course! Here's my portfolio: https://myportfolio.design", time: "9:30 AM", read: true },
        ],
    },
    {
        id: 3,
        ...
        online: false,
        messages: [
            { id: 1, from: "them", text: "Thank you for applying to the HR Specialist position.", time: "Mon", read: true },
            { id: 2, from: "them", text: "Your application is currently under review. We'll get back to you soon.", time: "Mon", read: true },
        ],
    },
];
...
function Avatar({ color, initials, online, size = "md" }) {
    ...
    return (
        <div className="relative flex-shrink-0">
            <div className={`${sizes[size]} ${color} rounded-full flex items-center justify-center font-semibold text-white`}>
                {initials}
            </div>
            {online && (
...
function ConversationItem({ convo, selected, onClick }) {
    return (
        ...
        >
            <Avatar color={convo.color} initials={convo.initials} online={convo.online} />
            <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-0.5">
                    <div className="flex items-center gap-1.5">
                        <span className="text-sm font-semibold text-gray-900 truncate">{convo.title}</span>
                        <span className="text-xs font-medium px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700">{convo.tag}</span>
                    </div>
                    <span className="text-xs text-gray-400 flex-shrink-0 ml-2">{convo.time}</span>
                </div>
                <div className="flex items-center justify-between">
                    <p className="text-xs text-gray-500 truncate pr-2">{convo.lastMessage}</p>
                    {convo.unread > 0 && (
            ...
            prev.map((c) =>
                c.id === selectedId
                    ? {
                          ...
                          lastMessage: input,
                          time: "Just now",
                          messages: [...c.messages, { id: Date.now(), from: "me", text: input, time: now, read: false }],
                      }
                    : c
            )
        );
        ...
        <div className="flex h-[80vh] bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200 font-sans">
            ...
            <div className={`w-80 flex-shrink-0 border-r border-gray-100 flex flex-col bg-gray-100 ${mobileView === "chat" ? "hidden sm:flex" : "flex"}`}>
                ...
                <div className="flex-1 overflow-y-auto">
                    {filtered.length === 0 ? (
                        <p className="text-sm text-gray-400 text-center mt-8">No conversations found</p>
                    ) : (
                        filtered.map((convo) => (
                            <ConversationItem
                                key={convo.id}
                    ...
                    <>
                        ...
                        <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-100 bg-white">
                            <div className="flex items-center gap-3">
                                ...
                                <Avatar color={selected.color} initials={selected.initials} online={selected.online} />
                                <div>
                                    <div className="flex items-center gap-2">
                                        <span className="text-sm font-semibold text-gray-900">{selected.title}</span>
                                        <span className="text-xs font-medium px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700">{selected.tag}</span>
                                    </div>
