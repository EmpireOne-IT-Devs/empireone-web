...
export async function get_job_application_by_id_service() {
    return await axios.get(
        `/api/job/application/${window.location.pathname.split("/")[4]}`,
    );
}

export async function get_job_application_by_user_service() {
    return (await axios.get(`/api/job/get_job_application_by_user`)).data;
}
